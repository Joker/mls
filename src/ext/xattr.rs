...
// mod from https://github.com/eza-community/eza/main/src/fs/feature/xattr.rs
...
pub fn list_attrs(lister: &lister::Lister, path: &Path) -> io::Result<Vec<Attribute>> {
	...
	for attr_name in buf.split(|c| c == &0) {
		...
		let c_attr_name = CString::new(attr_name).map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
		let size = lister.getxattr_first(&c_path, &c_attr_name);
...
mod lister {
	...
	extern "C" {
		fn listxattr(path: *const c_char, namebuf: *mut c_char, size: size_t, options: c_int) -> ssize_t;

		fn getxattr(
			path: *const c_char, name: *const c_char, value: *mut c_void, size: size_t, position: u32,
			options: c_int,
		) -> ssize_t;
	}
	...
	impl Lister {
		...
		pub fn listxattr_second(&self, c_path: &CString, buf: &mut [u8], bufsize: size_t) -> ssize_t {
			unsafe { listxattr(c_path.as_ptr(), buf.as_mut_ptr().cast(), bufsize, self.c_flags) }
		}
		...
		pub fn getxattr_second(
			&self, c_path: &CString, c_name: &CString, buf: &mut [u8], bufsize: size_t,
		) -> ssize_t {
			unsafe {
				getxattr(
					c_path.as_ptr(),
