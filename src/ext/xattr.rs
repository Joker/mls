...
// mod from https://github.com/ogham/exa/blob/master/src/fs/feature/xattr.rs
...
mod lister {
	...
	impl Lister {
		...
		pub fn listxattr_second(&self, c_path: &CString, buf: &mut Vec<u8>, bufsize: ssize_t) -> ssize_t {
			...
			unsafe { listxattr(c_path.as_ptr().cast(), buf.as_mut_ptr().cast(), bufsize as size_t) }
		}
		...
		pub fn getxattr(&self, c_path: &CString, buf: &[u8]) -> ssize_t {
			...
			unsafe { getxattr(c_path.as_ptr().cast(), buf.as_ptr().cast(), ptr::null_mut(), 0) }
		}
	}
}
